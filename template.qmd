...
```{r}
#| fig-cap: We would select 3 groups to initiate a kmeans cluster because the kink in the graph is most pronounced at three clusters
...
In the plot of Hubert index, we seek a significant knee that corresponds to a significant increase of the value of the measure
...
```{r}
# run in console
#seeds_km$size # 72, 71, 67# How many observations assigned to each cluster
#seeds_km$cluster : 169 rows# What cluster each observation in seeds_scale is assigned to
...
Now make a ggplot of area on the x-axis, asymmetric coefficient on the y-axis, color by the cluster numbers from kmeans, and use shape for the variety column.
...
```{r}
#| tbl-cap: Contiency table for the grouping of kneans and the varietals of kernels
seeds_cl  %>% 
  select(variety, cluster_no) %>% 
  table() %>% 
  knitr::kable()
```
accuracy visualized in table. 
...
- select only numeric
- scale the data 
- get the distance of the scaled data
- run hclust
- linkages either single or complete 
- make our dendrograms
...
wb_scaled <- wb_ghg_20 %>% 
  select(-c(name,region)) %>%  #deselect categorical columns
  scale()
...
```{r}
p_single + p_complete +  plot_layout(tag_level = 'new')

```


Single linkage branched out Sudan and Central African Republic, but not reflected on complete linkage. single linkage might be better in this case. Does it matter which method to choose?not really, just think about which one makes more sense. 
